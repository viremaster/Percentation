var slideX = {min: 8, max: 508};
var slideY = {min: 63, max: 563};
var mouse = {x: 0, y: 0};
let boxcounter = 0;
let item;
...
function pxtopercentx(pixels){
  ...
  return `${(pixels/slide.offsetWidth)*100}%`
}

function pxtopercenty(pixels){
  pixels = parseInt(pixels);
  return `${(pixels/slide.offsetHeight)*100}%`
}

function percenttopxx(percent){
  percent = parseInt(percent);
  return `${(percent/100)*slide.offsetWidth + slideX.min}px`
}

function percenttopxy(percent){
  percent = parseInt(percent);
  return `${(percent/100)*slide.offsetHeight + slideY.min}px`
}
